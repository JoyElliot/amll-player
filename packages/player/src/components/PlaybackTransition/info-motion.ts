...
export function createInfoMotion(
	...
	return {
		paint(fraction: number, progress: number) {
			...
			// Round the cover's lower corner before rising. Reverse the control
			// points on close so the text also clears the expanding cover edge.
			const lower = opening ? start : target;
			const upper = opening ? target : start;
			const outward = Math.min(48, Math.max(0, lower.left - upper.left) / 4);
			const below = Math.min(32, Math.abs(lower.top - upper.top) / 5);
			const nearLower = { x: lower.left + outward, y: lower.top + below };
			const nearUpper = { x: upper.left, y: lower.top + below };
			const first = opening ? nearLower : nearUpper;
			const second = opening ? nearUpper : nearLower;
			const u = 1 - fraction;
			current = {
				left:
					u ** 3 * start.left +
					3 * u ** 2 * fraction * first.x +
					3 * u * fraction ** 2 * second.x +
					fraction ** 3 * target.left,
				top:
					u ** 3 * start.top +
					3 * u ** 2 * fraction * first.y +
					3 * u * fraction ** 2 * second.y +
					fraction ** 3 * target.top,

...
整体时长为 500ms，卡片和封面使用 `cubic-bezier(.32,.72,.35,1)`。展开文字对这一进度取平方，使起步慢于封面，给封面先上移的空间，再在同一个 500ms 终点到达目标；收起仍使用原曲线。按钮上浮淡出仍为 160ms，收起仍在 320–500ms 下拉显现；卡片材质、分隔线和歌词页透明度保持原先时长及延时。测试同时测量初段文字相对封面的行程和真实文字交接时间，区分缓动调整与延长总时长。

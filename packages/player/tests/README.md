...
- 点击“视频封面”，等待本地测试视频生成，再运行。检查真实 video 节点保留、收起后两处视频暂停且时间一致；用例还会暂时保持缩略视频的 seeking 边界，验证交接等待时 resize 不会把文字临时样式写回。
- 点击“中途变更：关”启用中途变更再运行。用例在展开 90ms 后暂停音乐并切到沉浸布局，检查交接瞬间几何连续。
- 点击“切换”后，在动画进行中改变窗口横竖方向，再收起。检查仅一个全屏封面、无残留 popover/临时几何、焦点回到入口。
- 点击“长歌曲信息”后运行，确认长标题、多位歌手与文字浮层清理正常。
- 手动观察底栏分隔线上移、卡片材质渐变、封面位移、歌曲信息弧线、按钮上浮/下拉，以及阴影、沉浸遮罩和视频画面交接；Tab/Shift+Tab 不应进入非活动页面，Escape 收起。
...
视觉时序以 develop `c211fbde9ce0a8db2b3ab8037d798e6160fb942b` 为基准：卡片与封面使用 500ms `cubic-bezier(.25,1,.5,1)`；展开时底栏按钮上浮 36px 并在 160ms 内淡出，收起时等待 320ms 后用 180ms 下拉显现。卡片材质、分隔线与歌词页透明度分别保留原有延时。几何断言同时覆盖起点、中间帧和交接瞬间，视觉验收还需要检查实际动画帧。

按后续视觉调整，背景以顶部为缩放锚点。底栏真实歌曲信息节点进入 top layer，沿绕过封面下角后上升的三次贝塞尔曲线到达全屏信息位置，字号、行高和文字样式同步渐变；收起反向返回。`PrebuiltLyricPlayer.musicInfoProps` 只暴露当前布局的文字容器，不移动菜单按钮、不覆盖库的布局动画。响应式或沉浸布局更换锚点时保留当前姿态；不复制文字 DOM，不改 TextMarquee 内部动画。

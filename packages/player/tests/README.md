...
- 点击“中途变更：关”启用中途变更再运行。用例在展开约 130ms 后暂停音乐并切到沉浸布局，检查交接瞬间几何连续。
- 点击“切换”后，在动画进行中改变窗口横竖方向，再收起。检查仅一个全屏封面、无残留 popover/临时几何、焦点回到入口。
- 手动观察底栏分隔线上移、卡片材质渐变、封面位移、歌曲信息和按钮上浮/下拉，以及阴影、沉浸遮罩和视频画面交接；Tab/Shift+Tab 不应进入非活动页面，Escape 收起。
...
卡片结构和文字/按钮进出方式以 develop `c211fbde9ce0a8db2b3ab8037d798e6160fb942b` 为基准。按最新反馈，整体时长由 500ms 延长至 700ms，曲线改为 `cubic-bezier(.32,.72,.35,1)`，降低起步速度；展开时底栏文字与按钮上浮 36px 并在 220ms 内淡出，收起时等待 450ms 后用 250ms 下拉显现。卡片材质、分隔线和歌词页透明度按相同比例延长。几何断言同时覆盖起点、中间帧和交接瞬间，并检查 120ms 起步行程与 560ms 回程仍在继续，视觉验收还需要检查实际动画帧。

背景保留顶部缩放锚点及底栏封面按钮透明底色修复。歌曲信息恢复原有竖直平移淡入淡出，随卡片内的布局进入全屏；移除额外的文字浮层、贝塞尔路径及相关 ref 接入，不改 TextMarquee 内部动画。

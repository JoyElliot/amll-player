...
整体时长为 500ms，卡片和封面使用 `cubic-bezier(.32,.72,.35,1)`。展开文字继续对局部进度 `p` 取平方，沿直线慢起步。收起时纵向按 `1-(1-p)²` 先回位，横向单独基于原始时间使用 `cubic-bezier(.25,.1,.25,1)`，字号和尺寸随横向进度变化。这样横向在后段仍有可见移动，两个方向都在同一个 500ms 终点到位。按钮上浮淡出仍为 160ms，收起仍在 320–500ms 下拉显现；卡片材质、分隔线和歌词页透明度保持原先时长及延时。

背景保留顶部缩放锚点及底栏封面按钮透明底色修复。底栏的同一组歌曲名/歌手在 top layer 中从源位置移动到全屏文字位置；回程允许横纵分速，形成自然的纵向先行、横向跟进。不添加空间控制点、绕行或碰撞避让，也不改为源文字淡出后另一处文字淡入。公开的 `musicInfoProps` 提供当前布局目标，不改 TextMarquee 内部动画。

真实 DOM 检查分别验证展开共线、回程两轴单调且不越界；另检查回程初段纵向领先横向、400ms 横向仍有剩余行程，以及文字在 500ms 交接。中途反向、窗口变化或布局锚点更换时从当前姿态接续，原始时间重新从 0 开始；不保证重定向时速度连续。
